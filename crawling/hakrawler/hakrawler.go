...
func main() {
	...
	showCsv := flag.Bool("csv", false, "Output as CSV.")
	...
	proxyFlag := flag.String(("proxy"), "", "Proxy URL. E.g. -proxy http://127.0.0.1:8080")
	proxiesFlag := flag.String(("proxies"), "", "Comma-separated list of Proxy URLs. E.g. -proxies http://proxy1:8080,http://proxy2:8080")
	concurrentInputs := flag.Int("it", 1, "Number of concurrent stdin input URLs to process")
	...
	outFile := flag.String("out", "", "Output file to write results to")

	flag.Parse()

	if *proxyFlag != "" {
		os.Setenv("PROXY", *proxyFlag)
	}
	...
	go func() {
		...
		semaphore := make(chan struct{}, *concurrentInputs)
		for s.Scan() {
			urlLine := s.Text()
			wgInputs.Add(1)
			semaphore <- struct{}{}
			...
			c.OnHTML("a[href]", func(e *colly.HTMLElement) {
				...
				if strings.Contains(abs_link, url) || !*inside {
					...
					printResult(link, "href", *showSource, *showWhere, *showJson, *showCsv, results, e)
			...
			c.OnHTML("script[src]", func(e *colly.HTMLElement) {
				printResult(e.Attr("src"), "script", *showSource, *showWhere, *showJson, *showCsv, results, e)
			})

			// find and print all the form action URLs
			c.OnHTML("form[action]", func(e *colly.HTMLElement) {
				printResult(e.Attr("action"), "form", *showSource, *showWhere, *showJson, *showCsv, results, e)
			})
			...
			if *proxiesFlag != "" {
				proxyList := strings.Split(*proxiesFlag, ",")
				proxySwitcher, err := proxy.RoundRobinProxySwitcher(proxyList...)
				if err != nil {
					log.Println("Error parsing proxies:", err)
				} else {
	...
	defer w.Flush()
	if *showCsv {
		fmt.Fprintln(w, "Source,URL,Where")
	}
	urlsFound := false
	if *unique {
...
func printResult(link string, sourceName string, showSource bool, showWhere bool, showJson bool, showCsv bool, results chan string, e *colly.HTMLElement) {
	...
	if result != "" {
		if showJson {
			...
			cw := csv.NewWriter(&b)
			cw.Write([]string{sourceName, result, where})
			cw.Flush()
